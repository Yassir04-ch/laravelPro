    ...
    <div class="w-full max-w-md bg-white rounded-[2.5rem] shadow-xl border border-slate-100 p-8 md:p-10 relative overflow-hidden">
        ...
        <form action="{{ route('categories.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4 pt-2">
                <a href="{{ route('categories.index') }}" class="flex items-center justify-center py-4 bg-slate-100 text-slate-600 font-bold rounded-2xl hover:bg-slate-200 transition-all text-sm">
                    Cancel
                </a>
                <button type="submit" 
